...
try {
		...
		const katalog = (teks) => {
             res = pemuda.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_©Regad_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             pemuda.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = pemuda.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `${NamaBot}`, "footerText": "*_© Regad_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
pemuda.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
 ...
 if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© Regad_*", [{buttonId: 'simi', buttonText: {disText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
...
case 'command':
 ...
 footerText: '*_© Regad_*',
 description: `Hai kak @${stod.split('@')[0]}, Silahkan pilih menu disini, Kalau anda memakai Wa mod, dan gak muncul, Anda bisa mengetik .allmenu`,
 sections: [
                     {
...
case  'hentai': 
if (!isGroup) return reply(mess.only.group)
console.log('ADA YANG TELAH NGIRIM COMMAND HENTAI COEK')
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
reply(mess.wait)
anu = await fetchJson(`https://waifu.pics/api/nsfw/neko`)
buffer = await getBuffer(anu.url)
pemuda.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./pemuda_1.jpg')})
reply("Jangan Lupa Tobat ya😁")
break
case  'megumin':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
client.browserDescription = ['Regad','Desktop','3.0']
...
reply("itu owner/developer ku, ada perlu apa ye?")
break
case 'addcmd': 
case 'setcmd':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
break
		...
		case 'sc':
		case 'sourcecode':
		pemuda.sendMessage(from, { text: "Mau sc?, chat owner saya wa.me/6281455037288", matchedText: 'wa.me/6281455037288', description: "", title: "CIE MAU MINTA SC WOKWOKWOK", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
...
case 'x':
        ...
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        pemuda.toggleDisappearingMessages(from, 0)
        sendButLocation(from, `${ngazap(prefix)}`, `${virtag(prefix)}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}bbaij72njnwjibdo16830nslm1782`,buttonText:{disText:'⚔️ Regad ⚔️'},type:1}])
        pemuda.toggleDisappearingMessages(from, 0)
break
case 'bugrow':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
